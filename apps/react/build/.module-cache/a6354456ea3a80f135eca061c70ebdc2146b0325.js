...
var PhotoGrid = React.createClass({displayName: 'PhotoGrid',
	render: function() {
		...
		this.props.photos.forEach(function(photoModel) {
			if (photoModel.category !== lastCategory) {
				rows.push(React.createElement(ProductCategoryRow, {category: photoModel.category, key: photoModel.category}));
			}
			rows.push(React.createElement(Photo, {photo: photoModel, key: photoModel.name}));
			lastCategory = photoModel.category;
		});
		return (
